...
/* USER CODE BEGIN PV */
static uint8_t m_rtt_buffer[256];
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
static void TM_RTT_Read_Poll_Task(void);
static void TM_RTT_Send_Command(uint8_t * p_rtt_buffer, uint32_t length);
/* USER CODE END PFP */
...
/* USER CODE BEGIN TM_Init_2 */
  // Configure Up Buffer & Print Initial Message
  SEGGER_RTT_ConfigUpBuffer(0, NULL, NULL, 0, SEGGER_RTT_MODE_NO_BLOCK_SKIP);
  SEGGER_RTT_printf(0, "STM32WB55 Transparent Mode RTT %s\r\n", TM_RTT_VERSION);
  
  // Register & Start RTT Read Task
  UTIL_SEQ_RegTask(1<< CFG_TASK_RTT_READ_POLL_ID, UTIL_SEQ_RFU, TM_RTT_Read_Poll_Task);
  UTIL_SEQ_SetTask(1<< CFG_TASK_RTT_READ_POLL_ID, CFG_SCH_PRIO_0);
  ...
  if(HostTxStatus == TX_DONE)
  {
    ...
    if(LowPowerModeStatus == LOW_POWER_MODE_DISABLE)
    {
      ...
      if(pTxToHostPacket->evtserial.type == TL_ACL_DATA_PKT_TYPE)
      {
        ...
        //RESTORE_PRIMASK();
        p_target = (uint8_t *)&((TL_AclDataPacket_t *)pTxToHostPacket)->AclDataSerial;
        size = ((TL_AclDataPacket_t *)pTxToHostPacket)->AclDataSerial.length + 5;
      }
      else
      {
        /**
        ...
        //RESTORE_PRIMASK();
        p_target = (uint8_t *)&pTxToHostPacket->evtserial;
        size = pTxToHostPacket->evtserial.evt.plen + TL_EVT_HDR_SIZE;
      }
      
      for(uint32_t k = 0; k < size; k++)
      {
        sprintf(msg, "%s%02X", msg, p_target[k]);
      }
      
      /**
       * The interrupt shall be disable when the HAL is called to send data
       */
      DISABLE_IRQ();
...
static void TM_RTT_Read_Poll_Task(void)
{
  ...
  uint8_t add_to_buffer = 1;
  char c;
  
  // Read One Character from RTT
  uint32_t size = SEGGER_RTT_Read(0, &c, 1u);
  ...
  if(size > 0)
  {
    ...
    else if (c == '\r' || c == '\n')
    {
      if (buffer_length != 0)
      {
        // Send Command to CPU2 & Reset Buffer
        TM_RTT_Send_Command(m_rtt_buffer, buffer_length);
        memset(m_rtt_buffer, 0, sizeof(m_rtt_buffer));
        buffer_length = 0;
        msg_length = 0;
      }
    ...
    {
      SEGGER_RTT_printf(0, "ERROR: Non-Hex Value Entered\r\n");
      add_to_buffer = 0;
    }
    ...
    if (add_to_buffer == 1)
    {
      if (msg_length % 2 == 0)
        // Add MSB to Buffer
        m_rtt_buffer[buffer_length] = (uint8_t) (((c << 4) & 0x00F0));
      else
      { 
        // Add LSB to Buffer
        m_rtt_buffer[buffer_length] |= c;
        buffer_length++;
      }
...
static void TM_RTT_Send_Command(uint8_t * p_rtt_buffer, uint32_t length)
{
  uint8_t packet_indicator = p_rtt_buffer[0];
  uint8_t *p_target;
  
  switch(packet_indicator)
  {
    case TL_SYSCMD_PKT_TYPE:
      p_target = (uint8_t*)&(((TL_CmdPacket_t*)(p_RefTable->p_sys_table->pcmd_buffer))->cmdserial);
      UTIL_SEQ_SetTask(1<<CFG_TASK_SYS_HCI_CMD_ID,CFG_SCH_PRIO_0);
      break;

    case TL_LOCCMD_PKT_TYPE:
      p_target = (uint8_t*)&SysLocalCmd.cmdserial;
      UTIL_SEQ_SetTask(1<<CFG_TASK_SYS_LOCAL_CMD_ID,CFG_SCH_PRIO_0);
      break;

    case TL_ACL_DATA_PKT_TYPE:
      p_target = (uint8_t*)&(((TL_AclDataPacket_t*)(p_RefTable->p_ble_table->phci_acl_data_buffer))->AclDataSerial);
      UTIL_SEQ_SetTask(1<<CFG_TASK_HCI_ACL_DATA_ID,CFG_SCH_PRIO_0);
      break;

    default:
      p_target = (uint8_t*)&(((TL_CmdPacket_t*)(p_RefTable->p_ble_table->pcmd_buffer))->cmdserial);
      UTIL_SEQ_SetTask(1<<CFG_TASK_BLE_HCI_CMD_ID,CFG_SCH_PRIO_0);
      break;
  }
  
  memcpy(p_target, p_rtt_buffer, length);
}
